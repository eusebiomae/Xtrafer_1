...
<script id="tmplDiscount" type="text/x-dot-template">
	<div class="form-group" style="border-radius: 5px; border: 1px solid #ddd; padding-bottom: 5px" data-key="@{{= it.key }}">
		...
		<div class="col-sm-11">
			...
			<select name="discount[@{{= it.key }}][discount_id]" class="select2 form-control" value="@{{= it.discount_id }}"></select>
		</div>
...
<script>
	function newDiscount(data) {0
		...
		populateSelectBox({
			list: APP.listSelectBox.discount,
